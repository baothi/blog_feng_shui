...
class TuViForm(forms.Form):
    ...
    # name = forms.CharField(max_length=250, required=True,widget = forms.TextInput(attrs={'class': 'w1','placeholder' : "Input Full Name"}))
    # day_birthday = forms.ChoiceField(widget = forms.Select(),
    #     choices=((str(x), x) for x in range(1,32)), initial='14', required = True,)
    # month_birthday = forms.ChoiceField(widget = forms.Select(),
    #     choices=((str(x), x) for x in range(1,13)), initial='8', required = True,)
    # year_birthday = forms.ChoiceField(widget = forms.Select(),
    #     choices=((str(x), x) for x in range(1900,2044)), initial='1978', required = True,)
    # hours_birthday = forms.ChoiceField(widget = forms.Select(),
    #     choices=((str(x), x) for x in range(0,24)), initial='11', required = True,)
    # minute_birthday = forms.ChoiceField(widget = forms.Select(),
    ...
    #     choices=((str(x), x) for x in range(1,13)), initial='10', required = True,)
    # year = forms.ChoiceField(widget = forms.Select(),
    #     choices=((str(x), x) for x in range(1900,2044)), initial='2020', required = True,)
    # number_birthday = forms.ChoiceField(widget = forms.Select(),
    #     choices=((str(x), x) for x in range(0,6)), initial='11', required = True,)

    name = forms.CharField(max_length=250, required=False,widget = forms.TextInput(attrs={'class': 'form-control','placeholder' : "Input Full Name"}))
    day = forms.ChoiceField(widget = forms.Select(attrs={'class': 'form-control','text-align': 'center'}),
    ...
    minute = forms.ChoiceField(widget = forms.Select(attrs={'class': 'form-control'}),
        choices=((str(x), x) for x in range(0,60)), initial='12', required = True,)
    options = forms.ChoiceField(widget = forms.Select(attrs={'class': 'form-control'}),
                 choices = ([('Nam','Nam'), ('Nữ','Nữ'), ]), initial='Nam', required = True,)

    calendar = forms.ChoiceField(widget = forms.Select(attrs={'class': 'form-control'}),
                 choices = ([('Dương Lịch','Dương Lịch')]), initial='Dương Lịch', required = True,)
