...
def home(request):
    ...
    nhansohoc = Blog.objects.filter(is_available=True).filter(category__is_available=True).filter(category__slug="nhan-so-hoc").order_by('-created_date')[:3]
    vantrinhdoinguoi = Blog.objects.filter(is_available=True).filter(category__is_available=True).filter(category__slug="van-trinh-doi-nguoi").order_by('-created_date')[:3]
    kymon1 = Blog.objects.filter(is_available=True).filter(category__is_available=True).filter(category__slug="ky-mon").order_by('?')[2:3]
    kymon2 = Blog.objects.filter(is_available=True).filter(category__is_available=True).filter(category__slug="ky-mon").order_by('-created_date')[:3]
    ...
    list_product = Product.objects.filter(is_available=True).filter(category__is_available=True).order_by('?')[:10]
    # print(list_product)
    context = {
        ...
        'data': dataJSON,
        'nhansohoc': nhansohoc,
        'vantrinhdoinguoi': vantrinhdoinguoi,
        'kymon1': kymon1,
        'kymon2': kymon2,
